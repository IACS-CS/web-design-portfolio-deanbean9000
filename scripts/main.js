...
/* This JavaScript was modified with help from Github Copilot in response to the prompt "can you fix the code so the tabs snap back after they have reached the length to display the content" - 1/15/26 */
...
document.addEventListener("mousemove", function (event) {
  if (isDraggingTab && currentTabWrapper) {
    ...
    if (dragDistance > 0) {
      ...
      // If dragged 120px or more, show the content and snap back
      if (dragDistance >= 120) {
        ...
        if (section) {
          ...
          // Snap the tab back automatically
